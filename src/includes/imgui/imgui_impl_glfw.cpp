...
//  2023-07-18: Inputs: Revert ignoring mouse data on GLFW_CURSOR_DISABLED as it can be used
//  differently. User may set ImGuiConfigFLags_NoMouse if desired. (#5625, #6609) 2023-06-12: Accept
//  glfwGetTime() not returning a monotonically increasing value. This seems to happens on some
//  Windows setup when peripherals disconnect, and is likely to also happen on browser + Emscripten.
//  (#6491) 2023-04-04: Inputs: Added support for io.AddMouseSourceEvent() to discriminate
...
#include </Users/zstenger/.brew/Cellar/glfw/3.3.8/include/GLFW/glfw3native.h>  // for glfwGetCocoaWindow()
#endif
...
#ifdef _WIN32
	bd->GlfwWndProc =
		(WNDPROC)::GetWindowLongPtr((HWND)main_viewport->PlatformHandleRaw, GWLP_WNDPROC);
	IM_ASSERT(bd->GlfwWndProc != nullptr);
	::SetWindowLongPtr((HWND)main_viewport->PlatformHandleRaw, GWLP_WNDPROC,
